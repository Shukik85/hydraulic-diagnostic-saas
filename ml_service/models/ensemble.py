...
class EnsembleModel:
    ...
    async def predict(self, features: np.ndarray) -> dict[str, Any]:
        ...
        logger.info(
            "Processing ensemble prediction request",
                    ...
                    return {
                        **result,
                        ...
                        "ensemble_weights_used": {
                            p["ml_model"]: self.ensemble_weights.get(p["ml_model"], 0.0) for p in predictions
                        },
                    }
    ...
    async def _get_predictions_from_models(self, features: np.ndarray, model_list: list[str]) -> list[dict[str, Any]]:
        ...
        for model_name, result in zip(available_models, results):
            if isinstance(result, Exception):
                ...
                perf["avg_confidence"] = (
                    perf["avg_confidence"] * (perf["success_count"] - 1) + result.get("confidence", 0.0)
                ) / perf["success_count"]
                perf["avg_processing_time"] = (
                    perf["avg_processing_time"] * (perf["success_count"] - 1) + result.get("processing_time_ms", 0.0)
                ) / perf["success_count"]

        return predictions

    async def _predict_with_timeout(
        self, model: BaseMLModel, features: np.ndarray, model_name: str, timeout: float = 5.0
    ) -> dict[str, Any]:
        ...
        try:
            ...
            for model_name in self.ensemble_weights.keys():
                ...
                if perf["success_count"] > 0:
                    ...
                    performance_scores[model_name] = success_rate * 0.5 + avg_confidence * 0.3 + speed_score * 0.2
                else:
    ...
    def _update_metrics(self, strategy: str, inference_time: float, predictions: list[dict[str, Any]]) -> None:
        ...
        self.performance_metrics["fallback_usage"][strategy] = (
            self.performance_metrics["fallback_usage"].get(strategy, 0) + 1
        )
    ...
    async def warmup(self, warmup_samples: int = 10) -> None:
        ...
        logger.info("Ensemble warmup completed", performance_metrics=self.get_performance_metrics())
