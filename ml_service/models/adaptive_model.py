...
class AdaptiveModel(BaseMLModel):
        ...
        try:
            if model_path.exists():
                ...
                if isinstance(loaded_data, dict) and "base_model" in loaded_data:
                    ...
                    logger.info("Real Adaptive model loaded", 
    ...
    async def _adjust_threshold(self) -> None:
        ...
        if score_std > 0.5:  # High variance threshold
            ...
            new_threshold = np.clip(new_threshold, 
                                  self.base_threshold - max_adjustment,
                                  self.base_threshold + max_adjustment)
            ...
            if abs(new_threshold - self.adaptive_threshold) > 0.01:
                ...
                logger.debug("Threshold adjusted",
    ...
    def get_adaptation_stats(self) -> dict[str, Any]:
        ...
        return {
            ...
            "recent_anomaly_rate": sum(1 for p in recent_predictions if p['is_anomaly']) / max(len(recent_predictions), 1),
