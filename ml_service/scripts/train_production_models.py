...
class ProductionModelTrainer:
    def __init__(self, data_path: str = "data/uci_hydraulic/cycles_sample_100.parquet"):
        ...
        logger.info("ProductionModelTrainer initialized", data_path=str(self.data_path), models_dir=str(self.models_dir))
                ...
                if col.startswith(("PS", "TS", "FS", "VS", "EPS", "CE", "CP", "SE")):
                    if not pd.isna(row[col]):
                        readings.append(SensorReading(
                            ...
                            value=float(row[col]),
                            unit="bar" if col.startswith("PS") else "C" if col.startswith("TS") else "l/min" if col.startswith("FS") else "mm/s"
                        ))
    ...
    def train_random_forest(self, X_train: np.ndarray, y_train: np.ndarray, X_val: np.ndarray, y_val: np.ndarray) -> dict:
