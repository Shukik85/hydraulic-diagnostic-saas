...
class ModelTester:
    ...
    async def run_all_tests(self) -> Dict[str, Any]:
        ...
        all_results["summary"]["overall_success"] = (
            all_results["summary"]["models_passed"] >= 2 and  # At least 2 models work
            ensemble_results["load_success"]  # Ensemble loads
        )
...
Status: {summary_text}""",
        ...
        # Error Details
        if any(len(model_results["errors"]) > 0 for model_results in results["individual_models"].values()) or \
           len(ensemble_results["errors"]) > 0:
