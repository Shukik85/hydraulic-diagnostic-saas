...
class ProperTwoStageTrainer:
    ...
    def prepare_training_data(self, contract: Dict[str, Any]) -> tuple:
        ...
        logger.info("Features extracted according to contract",
                   features_used=X.shape[1],
                   contract_expected=expected_features)
    ...
    def train_stage1_binary(self, X_train: np.ndarray, y_train: np.ndarray,
        ...
        logger.info("Stage 1 binary model trained",
                   auc_score=auc_score,
    ...
    def train_stage2_multiclass(self, X_train: np.ndarray, y_train: np.ndarray,
        ...
        logger.info("Stage 2 multiclass model trained",
                   accuracy=accuracy,
                   oob_score=getattr(model, 'oob_score_', 'N/A'))
        
        return model, accuracy
    
    def save_models_and_metadata(self, binary_model: XGBClassifier, multiclass_model: RandomForestClassifier,
                                scaler: StandardScaler, contract: Dict[str, Any],
                                binary_auc: float, multiclass_accuracy: float) -> None:
    ...
    def train_full_pipeline(self) -> bool:
        ...
        try:
            ...
            binary_model, binary_auc = self.train_stage1_binary(
                X_train_scaled, y_bin_train, X_test_scaled, y_bin_test
            )
            ...
            self.save_models_and_metadata(
                binary_model, multiclass_model, scaler, contract,
                binary_auc, multiclass_accuracy
            )
            ...
            print(f"📊 ENTERPRISE PERFORMANCE:")
            ...
            print(f"🔄 Ready for API restart and Two-Stage activation!")
