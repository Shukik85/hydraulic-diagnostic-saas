...
class TwoStageTrainer:
    ...
    def train_stage2_multiclass(
        self, X_train: np.ndarray, y_train: np.ndarray, X_val: np.ndarray, y_val: np.ndarray, use_smote: bool = False
    ) -> dict:
        ...
        model.fit(
            X_train_balanced, y_train_balanced, sample_weight=sample_weight, eval_set=(X_val_faults, y_val_faults)
        )
    ...
    def train_two_stage_pipeline(self) -> dict:
        ...
        stage1_metadata, optimal_threshold = self.train_stage1_binary(X_train_scaled, y_train, X_val_scaled, y_val)

        # 5. Train Stage 2 (Multi-class) - Both variants
        stage2_plain = self.train_stage2_multiclass(X_train_scaled, y_train, X_val_scaled, y_val, use_smote=False)

        stage2_smote = self.train_stage2_multiclass(X_train_scaled, y_train, X_val_scaled, y_val, use_smote=True)
        ...
        try:
            ...
            best_multiclass = (
                self.version_dir / f"fault_classifier_catboost{'_smote' if best_stage2 == 'smote' else ''}.joblib"
            )
...
def main():
    ...
    try:
        ...
        print("  2. python scripts/push_to_api.py --cycles 10 --api http://localhost:8001")
