...
prediction_counter = Counter("ml_predictions_total", "Total number of predictions made", ["model", "result"])
prediction_latency = Histogram("ml_prediction_duration_seconds", "Time spent processing prediction requests")
...
class SimplePredictor:
    ...
    async def predict(self, features: list[float]) -> dict:
        ...
        ensemble_proba = sum(weights.get(name, 0.1) * proba for name, proba in probabilities.items())
        ensemble_proba /= sum(weights.get(name, 0.1) for name in probabilities)
...
@asynccontextmanager
async def lifespan_simple(_app: FastAPI):
...
if __name__ == "__main__":
    ...
    uvicorn.run("simple_predict:app", host="0.0.0.0", port=8001, reload=False)
