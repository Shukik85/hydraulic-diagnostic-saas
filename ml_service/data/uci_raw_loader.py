...
def export_processed(df: pd.DataFrame):
    ...
    tidy_cols = {
        ...
        "Mean Temp": df.get("TS1", pd.Series(np.random.normal(60, 3, len(df)))).rolling(50, min_periods=1).mean()
        if "TS1" in df.columns
        else pd.Series(np.random.normal(60, 3, len(df))),
