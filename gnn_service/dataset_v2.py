"""
Enhanced Dataset with OPTIMIZED Data Augmentation

Changes from original:
- Reduced noise: 0.01 (was 0.02)
- Reduced scale range: (0.98, 1.02) (was 0.95-1.05)
- Reduced edge dropout: 0.02 (was 0.05)
"""
...
class DataAugmentation:
    """Optimized data augmentation for hydraulic graph data."""

    def __init__(
        self,
        noise_std: float = 0.01,  # ✅ Reduced from 0.02
        scale_range: tuple = (0.98, 1.02),  # ✅ Reduced from (0.95, 1.05)
        edge_dropout: float = 0.02,  # ✅ Reduced from 0.05
        enabled: bool = True,
    ):
...
class EnhancedHydraulicGraphDataset(Dataset):
    """Enhanced dataset with optimized augmentation."""
...
def split_dataset(
    ...
    """Split dataset with optimized augmentation."""
    ...
    # ✅ Training set: WITH optimized augmentation
    train_dataset = EnhancedHydraulicGraphDataset(
        graphs=train_graphs,
        augmentation=DataAugmentation(
            noise_std=0.01,  # ✅ Reduced
            scale_range=(0.98, 1.02),  # ✅ Reduced
            edge_dropout=0.02,  # ✅ Reduced
            enabled=True,
        ),
        training=True,
    )
...
def create_data_loaders(
    ...
    """Create train/val/test data loaders with optimized augmentation."""
