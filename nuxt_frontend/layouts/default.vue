...
<template>
  <div class="min-h-screen bg-gray-50 dark:bg-gray-900">
    ...
    <nav class="sticky top-0 z-50 bg-white/95 dark:bg-gray-900/95 backdrop-blur-sm border-b border-gray-200 dark:border-gray-800 shadow-sm will-change-transform">
      <div class="container mx-auto flex items-center justify-between h-16 px-6">
        <NuxtLink to="/" class="flex items-center space-x-3 mr-12">
          ...
          <div>
            ...
            <span class="block text-xs text-gray-500 dark:text-gray-400 leading-tight">Диагностическая платформа</span>
          </div>
        </NuxtLink>
        ...
        <div class="flex items-center space-x-4">
          <button @click="toggleTheme" class="p-2 text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-100 hover:bg-gray-100 dark:hover:bg-gray-800 rounded-lg transition-colors" title="Переключить тему">
            <Icon :name="isDark ? 'heroicons:sun' : 'heroicons:moon'" class="w-5 h-5" />
          </button>
          <NuxtLink to="/dashboard" class="px-6 py-2.5 text-sm font-bold text-white bg-gradient-to-r from-blue-600 to-purple-600 rounded-lg hover:from-blue-700 hover:to-purple-700 shadow-lg hover:shadow-xl transition-all duration-200">Открыть дашборд</NuxtLink>
        </div>
...
:deep(header), :deep(.navbar), :deep(.site-nav) {
