        ...
        <nav class="flex-1 space-y-1 px-3 py-4">
          <template v-for="item in navigationItems" :key="item.name">
            <NuxtLink v-if="!item.isExternal" :to="item.href" @click="isMobileMenuOpen = false" :class="[
              ...
              $route.path === item.href ? 'bg-gradient-to-r from-blue-600 to-blue-700 text-white shadow-md' : 'text-gray-700 hover:bg-blue-50 hover:text-blue-700 dark:text-gray-200 dark:hover:bg-blue-900/30 dark:hover:text-blue-300'
            ]">
              <Icon :name="item.icon" class="mr-3 h-5 w-5 flex-shrink-0" />
              {{ item.name }}
            </NuxtLink>
            <NuxtLink v-else :to="item.href" class="w-full group flex items-center rounded-lg px-3 py-3 text-sm font-medium transition-colors text-gray-700 hover:bg-green-50 hover:text-green-700 dark:text-gray-200 dark:hover:bg-green-900/30 dark:hover:text-green-300 border border-dashed border-green-200 dark:border-green-700">
          ...
          <ul role="list" class="flex flex-1 flex-col gap-y-2">
            <li v-for="item in navigationItems" :key="item.name">
              <NuxtLink v-if="!item.isExternal" :to="item.href" :class="[
                'group flex gap-x-3 rounded-xl p-4 text-sm font-semibold leading-6 transition-all duration-200',
    ...
    <div class="lg:pl-80">
      ...
      <div class="sticky top-0 z-40 bg-white/96 dark:bg-gray-900/96 backdrop-blur-md border-b border-gray-200 dark:border-gray-700 shadow-sm">
        <div class="flex h-16 items-center gap-x-4 px-4 sm:gap-x-6 sm:px-6 lg:px-8">
          <button type="button" class="-m-2.5 p-2.5 text-gray-700 dark:text-gray-300 lg:hidden hover:bg-gray-100 dark:hover:bg-gray-700 rounded-lg" @click="isMobileMenuOpen = true">
          ...
          <nav class="flex items-center space-x-2 text-sm flex-1" aria-label="Breadcrumb">
            <template v-for="(crumb, index) in breadcrumbs" :key="crumb.href">
              <NuxtLink v-if="index < breadcrumbs.length - 1" :to="crumb.href" class="font-medium text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200 transition-colors">
                {{ crumb.name }}
              </NuxtLink>
              <span v-else class="font-bold text-gray-900 dark:text-white">{{ crumb.name }}</span>
          ...
          <div class="flex items-center gap-x-4">
            ...
            <PremiumButton to="/diagnostics" size="sm" icon="heroicons:plus" gradient class="hidden sm:inline-flex">
              Новая диагностика
            </PremiumButton>
          </div>
        </div>
