          ...
          <ul role="list" class="flex flex-1 flex-col gap-y-2">
            <li v-for="item in navigationItems" :key="item.name">
              <NuxtLink v-if="!item.isExternal" :to="item.href" :class="[
                'group flex gap-x-3 rounded-xl p-4 text-sm font-semibold leading-6 transition-all duration-200 hover:scale-[1.02]',
                $route.path === item.href
                  ? 'bg-gradient-to-r from-blue-600 to-blue-700 text-white shadow-lg transform'
