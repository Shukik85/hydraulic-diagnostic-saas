    ...
    <div class="container mx-auto px-4 py-8 space-y-6">
      ...
      <div class="premium-card">
        <div class="p-6 border-b border-gray-200 dark:border-gray-700">
          <h3 class="premium-heading-sm text-gray-900 dark:text-white">📊 Recent Diagnostic Results</h3>
          <p class="premium-body text-gray-600 dark:text-gray-300">
            ...
            >
              ...
              <div class="flex items-center space-x-4">
                <div class="text-right">
                  <p class="text-sm font-medium text-gray-900 dark:text-white">{{ result.score }}/100</p>
      ...
      <div v-if="showRunModal" class="fixed inset-0 bg-black/50 z-50 flex items-center justify-center" @click="showRunModal = false">
        <div class="premium-card max-w-md w-full m-4" @click.stop>
          ...
          <div class="p-6 border-t border-gray-200 dark:border-gray-700 flex space-x-3">
            ...
            <PremiumButton :disabled="!selectedEquipment" @click="startDiagnostic" class="flex-1" icon="heroicons:play">
              Start Diagnostic
            </PremiumButton>
          </div>
        </div>
      ...
      <div v-if="showResultsModal" class="fixed inset-0 bg-black/50 z-50 flex items-center justify-center" @click="showResultsModal = false">
          ...
          <div class="p-6 space-y-6">
            ...
            <div class="grid gap-4 md:grid-cols-3">
              <div class="premium-card p-4 text-center">
                <div class="text-2xl font-bold text-green-600 dark:text-green-400">{{ selectedResult?.score }}/100</div>
                <p class="text-sm text-gray-500 dark:text-gray-400">Overall Health Score</p>
              </div>
              <div class="premium-card p-4 text-center">
                <div class="text-2xl font-bold text-gray-900 dark:text-white">{{ selectedResult?.issuesFound }}</div>
                <p class="text-sm text-gray-500 dark:text-gray-400">Issues Found</p>
              </div>
              <div class="premium-card p-4 text-center">
                <div class="text-2xl font-bold text-gray-900 dark:text-white">{{ selectedResult?.duration }}</div>
            ...
            <div class="premium-card p-6">
              <h4 class="premium-heading-sm text-gray-900 dark:text-white mb-4">📝 Recommendations</h4>
              <div class="space-y-4">
                <div class="p-4 border border-yellow-200 dark:border-yellow-800 bg-yellow-50 dark:bg-yellow-900/20 rounded-lg">
                  <div class="flex items-start space-x-3">
                    <Icon name="heroicons:exclamation-triangle" class="h-5 w-5 text-yellow-600 dark:text-yellow-400 mt-0.5" />
                    <div>
                      <p class="font-medium text-yellow-800 dark:text-yellow-200">Pressure System Maintenance</p>
                      <p class="text-sm text-yellow-700 dark:text-yellow-300">
                        Schedule filter replacement within 2 weeks to prevent pressure fluctuations.
                      </p>
                      <p class="text-xs text-yellow-600 dark:text-yellow-400 mt-1">Priority: Medium</p>
                    </div>
                  </div>
                </div>
                ...
                <div class="p-4 border border-green-200 dark:border-green-800 bg-green-50 dark:bg-green-900/20 rounded-lg">
                  <div class="flex items-start space-x-3">
                    <Icon name="heroicons:check-circle" class="h-5 w-5 text-green-600 dark:text-green-400 mt-0.5" />
                    <div>
                      <p class="font-medium text-green-800 dark:text-green-200">Temperature Monitoring</p>
