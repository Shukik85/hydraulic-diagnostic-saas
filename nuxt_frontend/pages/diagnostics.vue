...
const startDiagnostic = async (data: any) => {
  ...
  const interval = setInterval(() => {
    ...
    if (session.progress >= 100) {
      ...
      setTimeout(() => {
        activeSessions.value = activeSessions.value.filter((s: { id: number }) => s.id !== session.id)
...
const cancelSession = (sessionId: number) => {
  activeSessions.value = activeSessions.value.filter((s: { id: number }) => s.id !== sessionId)
...
const viewResult = (resultId: number) => {
  selectedResult.value = recentResults.value.find((r: { id: number }) => r.id === resultId)
...
const getStatusBadgeClass = (status: string) => {
  ...
  return classes[status as keyof typeof classes] || 'u-badge-gray'
}
...
const getStatusIcon = (status: string) => {
  ...
  return icons[status as keyof typeof icons] || 'heroicons:question-mark-circle'
}
</script>