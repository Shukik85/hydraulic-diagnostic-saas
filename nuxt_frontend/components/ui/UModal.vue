            ...
            <div v-if="$slots.header || title" class="border-b border-gray-200 px-6 py-4">
              <slot name="header">
                <div>
                  ...
                  <p v-if="description" :id="descriptionId" class="text-sm text-gray-500 mt-1">{{ description }}</p>
                </div>
              </slot>
            </div>
...
watch(() => props.modelValue, (isOpen) => {
  if (isOpen) nextTick(() => modalRef.value?.focus())
})
</script>
