    ...
    <div class="premium-card p-4 flex items-center justify-between">
      <div class="flex items-center space-x-3">
        ...
        <div class="flex rounded-lg overflow-hidden border border-gray-200 dark:border-gray-700">
          <button
            :class="['px-4 py-2 text-sm font-medium', source==='btc' ? 'bg-blue-600 text-white' : 'bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-300']"
            @click="source='btc'"
          >BTC</button>
          <button
            :class="['px-4 py-2 text-sm font-medium', source==='eth' ? 'bg-blue-600 text-white' : 'bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-300']"
            @click="source='eth'"
          >ETH</button>
        </div>
      </div>
    ...
    <div class="premium-card p-4">
      <div class="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
        <div>
          ...
          <div class="flex items-center space-x-3">
            <span :class="zoneColor('temperature', weekStats.temperature?.avg)">ср: {{ weekStats.temperature?.avg }}</span>
            <span class="text-gray-400">/</span>
            <span class="text-gray-600 dark:text-gray-300">мин: {{ weekStats.temperature?.min }}</span>
            <span class="text-gray-400">/</span>
            <span class="text-gray-600 dark:text-gray-300">макс: {{ weekStats.temperature?.max }}</span>
          </div>
        </div>
        <div>
          <div class="text-gray-500 dark:text-gray-400 mb-1">Давление (бар)</div>
          <div class="flex items-center space-x-3">
            <span :class="zoneColor('pressure', weekStats.pressure?.avg)">ср: {{ weekStats.pressure?.avg }}</span>
            <span class="text-gray-400">/</span>
            <span class="text-gray-600 dark:text-gray-300">мин: {{ weekStats.pressure?.min }}</span>
            <span class="text-gray-400">/</span>
            <span class="text-gray-600 dark:text-gray-300">макс: {{ weekStats.pressure?.max }}</span>
          </div>
        </div>
        <div>
          <div class="text-gray-500 dark:text-gray-400 mb-1">Расход (л/мин)</div>
          <div class="flex items-center space-x-3">
            <span :class="zoneColor('flow_rate', weekStats.flow_rate?.avg)">ср: {{ weekStats.flow_rate?.avg }}</span>
            <span class="text-gray-400">/</span>
            <span class="text-gray-600 dark:text-gray-300">мин: {{ weekStats.flow_rate?.min }}</span>
            <span class="text-gray-400">/</span>
            <span class="text-gray-600 dark:text-gray-300">макс: {{ weekStats.flow_rate?.max }}</span>
          </div>
        </div>
        <div>
          <div class="text-gray-500 dark:text-gray-400 mb-1">Вибрация (мм/с)</div>
          <div class="flex items-center space-x-3">
            <span :class="zoneColor('vibration', weekStats.vibration?.avg)">ср: {{ weekStats.vibration?.avg }}</span>
            <span class="text-gray-400">/</span>
            <span class="text-gray-600 dark:text-gray-300">мин: {{ weekStats.vibration?.min }}</span>
            <span class="text-gray-400">/</span>
            <span class="text-gray-600 dark:text-gray-300">макс: {{ weekStats.vibration?.max }}</span>
          </div>
        </div>
      </div>
    </div>
