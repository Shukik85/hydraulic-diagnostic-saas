...
def check_celery() -> dict[str, Any]:
    ...
    if not CELERY_AVAILABLE:
        return {"status": "unavailable", "error": "Celery not installed or configured"}
...
def check_ai_services() -> dict[str, Any]:
    ...
    try:
        ...
        if not openai_key:
            return {"status": "unavailable", "warning": "OpenAI API key not configured"}
...
def health_check(request):
    ...
    if include_detailed:
        ...
        performance_metrics = {
            ...
            "ai_requests_last_hour": cache.get(
                f"ai_metrics:{time.strftime('%Y-%m-%d:%H')}:ai_requests", 0
            ),
        }
