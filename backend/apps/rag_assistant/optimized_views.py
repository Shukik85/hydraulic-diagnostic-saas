...
from .tasks import process_document_async, index_documents_batch_async
    ...
    def get_queryset(self):
        ...
        if self.action == 'list':
            ...
            queryset = queryset.only(
                'id', 'title', 'language', 'format', 'created_at', 'updated_at',
                'rag_system__id', 'rag_system__name'
            )
...
class OptimizedRagSystemViewSet(viewsets.ModelViewSet):
    ...
    queryset = RagSystem.objects.prefetch_related(
        Prefetch('document_set', queryset=Document.objects.only('id', 'title', 'language', 'format'))
    ).annotate(
...
class OptimizedRagQueryLogViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        ...
        if self.action == 'list':
            queryset = queryset.only(
                'id', 'query_text', 'timestamp', 'system__name',
                'document__title'
            )
