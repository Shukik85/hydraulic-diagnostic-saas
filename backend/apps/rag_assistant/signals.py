...
from .models import KnowledgeBase, DocumentChunk, RAGSystemSettings
...
def document_chunk_created(sender, instance, created, **kwargs):
    ...
    if created:
        ...
        logger.debug(f"Создан фрагмент {instance.chunk_index} для документа {instance.document.title}")
