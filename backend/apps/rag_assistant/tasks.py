...
def process_document_async(self, document_id: int, reindex: bool = False) -> Dict[str, Any]:
    ...
    try:
        with task_performance_monitor('process_document', document_id=document_id, reindex=reindex):
            ...
            self.update_state(
                state='PROGRESS',
                meta={'current': 20, 'total': 100, 'status': 'Document loaded, initializing RAG assistant...'}
            )
            ...
            self.update_state(
                state='PROGRESS',
                meta={'current': 40, 'total': 100, 'status': 'Processing and indexing document...'}
            )
            ...
            self.update_state(
                state='PROGRESS',
                meta={'current': 80, 'total': 100, 'status': 'Updating cache and metadata...'}
            )
            ...
            self.update_state(
                state='SUCCESS',
                meta={'current': 100, 'total': 100, 'status': 'Document processing completed successfully'}
            )
        ...
        with task_performance_monitor('batch_index_documents', total_docs=total_docs):
            ...
            for doc in documents:
                if doc.rag_system.id not in docs_by_system:
                    docs_by_system[doc.rag_system.id] = {'system': doc.rag_system, 'docs': []}
                docs_by_system[doc.rag_system.id]['docs'].append(doc)
            
            logger.info(f"Starting batch processing of {total_docs} documents across {len(docs_by_system)} systems")
            ...
            for system_id, system_data in docs_by_system.items():
                ...
                try:
                    ...
                    logger.error(f"Error initializing RAG assistant for system {system_id}: {str(e)}")
...
def generate_performance_report() -> Dict[str, Any]:
    ...
    try:
        with task_performance_monitor('performance_report'):
            ...
            logger.info(f"Performance report generated: {total_requests} requests, {total_ai_requests} AI operations")
