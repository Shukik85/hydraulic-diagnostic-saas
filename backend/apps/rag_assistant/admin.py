...
class KnowledgeBaseAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        (
            "Содержимое",
            {"fields": ("content", "summary", "keywords"), "classes": ("collapse",)},
        ),
        (
            "Статистика",
            {
...
class RAGQueryAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ("Запрос", {"fields": ("user", "query_text", "query_type", "status")}),
        (
            "Ответ",
            {
                "fields": (
        ...
        ("Оценка пользователя", {"fields": ("user_rating", "user_feedback")}),
        (
            "Метаданные",
...
class RAGSystemSettingsAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ("Модель эмбеддингов", {"fields": ("embedding_model", "embedding_dimensions")}),
        ("Параметры поиска", {"fields": ("search_top_k", "similarity_threshold")}),
        ("Параметры чанкинга", {"fields": ("chunk_size", "chunk_overlap")}),
        ("Параметры генерации", {"fields": ("max_response_tokens", "temperature")}),
        ("Кэширование", {"fields": ("enable_caching", "cache_ttl_hours")}),
