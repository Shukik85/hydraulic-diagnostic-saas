"""Core diagnostic models - extracted from main models.py for better organization."""
...
class HydraulicSystemQuerySet(models.QuerySet["HydraulicSystem"]):
    """QuerySet for HydraulicSystem with optimized queries."""

    def with_owner(self) -> HydraulicSystemQuerySet:
        """Select related owner for efficient queries.

        Returns:
            QuerySet with owner prefetched
        """
    ...
    def active(self) -> HydraulicSystemQuerySet:
        ...
        Returns:
            QuerySet with active systems only
        """
        return self.filter(status="active")


class HydraulicSystem(models.Model):
    """Hydraulic system model representing a complete hydraulic installation.

    This is the main entity that contains all components, sensors, and diagnostic data.
    """
...
class SystemComponentQuerySet(models.QuerySet["SystemComponent"]):
    """QuerySet for SystemComponent."""

    def for_system(self, system_id: uuid.UUID) -> SystemComponentQuerySet:
        """Filter components for specific system.
...
class SystemComponent(models.Model):
    """System component representing equipment within a hydraulic system."""
...
class SensorDataQuerySet(models.QuerySet["SensorData"]):
    """QuerySet for SensorData with time-series optimizations."""

    def for_system(self, system_id: uuid.UUID) -> SensorDataQuerySet:
        """Filter data for specific system.

        Args:
            system_id: UUID of the system

        Returns:
            Filtered QuerySet with prefetched component
        """
...
class SensorData(models.Model):
    """Sensor data model for storing time-series measurement data.
    ...
    def clean(self) -> None:
        ...
        Raises:
            ValidationError: If timestamp is too far in the future
        """
    ...
    def save(self, *args, **kwargs) -> None:  # type: ignore[no-untyped-def]
        """Save the sensor data and update system's last reading timestamp.
...
class DiagnosticReportQuerySet(models.QuerySet["DiagnosticReport"]):
    ...
    def recent_for_system(
        ...
        """Get recent reports for a system.
...
class DiagnosticReport(models.Model):
    """Diagnostic report for analysis results and recommendations."""
    ...
    def clean(self) -> None:
        ...
        Raises:
            ValidationError: If AI confidence is out of valid range
        """
