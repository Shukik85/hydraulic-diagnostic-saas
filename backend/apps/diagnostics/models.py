...
class HydraulicSystem(models.Model):
    ...
    system_type = models.CharField(
        max_length=50, choices=SYSTEM_TYPES,
        verbose_name='Тип системы', db_index=True
    )
    status = models.CharField(
        max_length=20, choices=STATUS_CHOICES,
        default='active', verbose_name='Статус', db_index=True
    )
    criticality = models.CharField(
        max_length=20, choices=CRITICALITY_LEVELS,
        default='medium', verbose_name='Критичность', db_index=True
    )
    ...
    def needs_maintenance(self):
        # Предполагается наличие поля next_maintenance
...
class SensorData(models.Model):
    ...
    system = models.ForeignKey(
        HydraulicSystem, on_delete=models.CASCADE,
        related_name='sensor_data', verbose_name='Система', db_index=True
    )
    component = models.ForeignKey(
        SystemComponent, on_delete=models.CASCADE,
        related_name='sensor_data', verbose_name='Компонент', db_index=True
    )
...
class DiagnosticReport(models.Model):
    ...
    system = models.ForeignKey(
        HydraulicSystem, on_delete=models.CASCADE,
        related_name='diagnostic_reports', verbose_name='Система', db_index=True
    )
    title = models.CharField(max_length=200, verbose_name='Заголовок')
    severity = models.CharField(
        max_length=20, choices=SEVERITY_CHOICES,
        verbose_name='Важность', db_index=True
    )
    status = models.CharField(
        max_length=20, choices=STATUS_CHOICES,
...
class MaintenanceSchedule(models.Model):
    ...
    def __str__(self):
        return f"{self.system.name} на {self.schedule_date}"
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    name = models.CharField(max_length=200, verbose_name='Название системы')
    description = models.TextField(blank=True, verbose_name='Описание')
