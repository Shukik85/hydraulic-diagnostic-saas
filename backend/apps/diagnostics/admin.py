...
class HydraulicSystemAdmin(admin.ModelAdmin):
    ...
    list_display = [
        'name', 'system_type', 'status', 'owner', 'location', 
        'max_pressure', 'max_flow', 'health_indicator', 
        'last_sensor_data', 'created_at'
    ]
    
    list_filter = [
        'system_type', 'status', 'created_at', 'owner',
        ('installation_date', admin.DateFieldListFilter)
    ]
    
    search_fields = ['name', 'location', 'owner__username', 'owner__email']
    
    readonly_fields = ['created_at', 'updated_at', 'health_indicator', 'system_statistics']
    ...
    fieldsets = (
        ...
        ('Технические характеристики', {
            'fields': ('max_pressure', 'max_flow', 'temperature_range', 'installation_date')
        }),
        ('Системная информация', {
            'fields': ('created_at', 'updated_at', 'health_indicator', 'system_statistics'),
            'classes': ('collapse',)
        })
    )
    ...
    def health_indicator(self, obj):
        ...
        try:
            ...
            if not recent_data.exists():
                return format_html(
                    '<span style="color: gray;">⚪ Нет данных</span>'
                )
    ...
    def system_statistics(self, obj):
        ...
        try:
            ...
            critical_count = obj.sensor_data.filter(timestamp__gte=week_ago, is_critical=True).count()
...
class SensorDataAdmin(admin.ModelAdmin):
    ...
    list_display = [
        'system', 'sensor_type', 'value_with_unit', 'timestamp', 
        'critical_indicator', 'system_owner'
    ]
    
    list_filter = [
        'sensor_type', 'is_critical', 'system__system_type',
        ('timestamp', admin.DateFieldListFilter),
        'system__owner'
    ]
    
    search_fields = [
        'system__name', 'system__owner__username', 
        'warning_message', 'sensor_type'
    ]
    ...
    def mark_as_critical(self, request, queryset):
        ...
        updated = queryset.update(is_critical=True, warning_message='Отмечено как критическое администратором')
...
class DiagnosticReportAdmin(admin.ModelAdmin):
    ...
    list_display = [
        'title', 'system', 'severity_indicator', 'system_owner', 
        'has_ai_analysis', 'created_at'
    ]
    
    list_filter = [
        'severity', 'system__system_type', 'system__owner',
    ...
    def regenerate_ai_analysis(self, request, queryset):
        ...
        for report in queryset:
            try:
                ...
                self.message_user(request, f"Ошибка регенерации для {report.title}: {e}", level='ERROR')
