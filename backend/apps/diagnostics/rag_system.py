...
class HydraulicKnowledgeBase:
    ...
    def _load_base_knowledge(self):
        ...
        base_knowledge = [
            {
                ...
                падение давления более 10%, повышение температуры выше 80°C, появление металлической стружки в масле.''',
                'category': 'standards',
        ...
        for doc in base_knowledge:
            cursor.execute('''
                ...
                VALUES (?, ?, ?, ?, ?)
            ''', (doc['title'], doc['content'], doc['category'], doc['tags'], doc['source']))
    ...
    def get_diagnostic_recommendations(self, symptoms: List[str], system_type: str = None) -> List[Dict]:
    ...
    def add_diagnostic_case(self, problem: str, symptoms: List[str], solution: str, 
                          system_type: str = None, success_rate: float = 1.0):
    ...
    def generate_contextual_answer(self, question: str, context_data: Dict = None) -> Dict[str, Any]:
    ...
    def _extract_recommendations(self, content: str) -> str:
        ...
        keywords = ['рекомендуется', 'необходимо', 'следует', 'проверить', 'заменить', 'контролировать']
        ...
        try:
            ...
            for doc in new_documents:
                cursor.execute('''
                    ...
                    (title, content, category, tags, source) 
                    VALUES (?, ?, ?, ?, ?)
                ''', (doc['title'], doc['content'], doc.get('category', 'general'),
                     doc.get('tags', ''), doc.get('source', 'user')))
    ...
    def get_knowledge_statistics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                'vector_dimensions': self.vectorizer.max_features if hasattr(self.vectorizer, 'max_features') else 0,
