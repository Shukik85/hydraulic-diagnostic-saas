...
def main():
    ...
    hs = HydraulicSystem.objects.create(name="Test System", system_type="industrial", status="active")
    ...
    ts1 = timezone.now()
    SensorData.objects.create(system=hs, component=comp, timestamp=ts1, unit="bar", value=100.0)
    hs.refresh_from_db()
    print("last_reading_at after first reading:", hs.last_reading_at.isoformat())

    ts2 = timezone.now()
    SensorData.objects.create(system=hs, component=comp, timestamp=ts2, unit="bar", value=101.5)
    hs.refresh_from_db()
    ...
    hs2 = HydraulicSystem.objects.create(name="Second System", system_type="industrial", status="active")
