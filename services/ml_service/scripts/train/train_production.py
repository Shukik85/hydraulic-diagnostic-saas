...
GRIDS = {
    "catboost": {
        "cpu_fast": {"iterations": [300], "depth": [6], "learning_rate": [0.1], "l2_leaf_reg": [3]},
        "gpu_full": {
...
class RealProductionModelTrainer:
    ...
    def __init__(self, use_gpu: bool = False, mode: str = "fast"):
        ...
        if use_gpu:
            ...
            console.print(
                f"   cuML acceleration: {'✅ Available' if CUML_AVAILABLE else '❌ Install: pip install cuml-cu12'}"
            )
        else:
    ...
    def load_real_data(self, data_path: str = None) -> dict[str, np.ndarray]:
        ...
        try:
            data = load_uci_hydraulic_data(filename=data_path or "Industrial_fault_detection.csv", window_minutes=5)
            console.print(f"✅ Loaded dataset: {data['X_train'].shape[0]} training samples")
        except Exception as e:
            console.print(f"⚠️  Primary dataset failed: {e}")
            data = load_uci_hydraulic_data(filename="industrial_fault_detection_data_1000.csv", window_minutes=5)
    ...
    def train_catboost_real(self, X_train, y_train, X_val, y_val) -> dict[str, Any]:
        ...
        mode_text = (
            f"GPU Full ({combinations} combinations)" if self.use_gpu else f"CPU Fast ({combinations} combinations)"
        )
        console.print(f"\n🐱 CatBoost {mode_text}")
        ...
        grid_search = GridSearchCV(catboost_base, param_grid, cv=3, scoring="roc_auc", n_jobs=n_jobs, verbose=0)
    ...
    def train_xgboost_real(self, X_train, y_train, X_val, y_val) -> dict[str, Any]:
        ...
        base_params = {"random_state": 42, "eval_metric": "auc", "verbosity": 0}
        ...
        grid_search = GridSearchCV(xgb_base, param_grid, cv=3, scoring="roc_auc", n_jobs=n_jobs, verbose=0)
        ...
        try:
            ...
            final_model.fit(X_train_gpu, y_train_gpu, eval_set=[(X_val_gpu, y_val_gpu)], verbose=False)
        except Exception:
    ...
    def train_random_forest_real(self, X_train, y_train, X_val, y_val) -> dict[str, Any]:
        ...
        mode_text = (
            f"GPU Full ({combinations} combinations){cuml_text}"
            if self.use_gpu
            else f"CPU Fast ({combinations} combinations)"
        )
        ...
        grid_search = GridSearchCV(
            ...
            scoring="roc_auc",
            n_jobs=1,
            verbose=0,  # Single job for grid to avoid conflicts
        )
                ...
                for max_feat in param_grid["max_features"]:
                    try:
                        model = IsolationForest(
                            n_estimators=n_est, contamination=contam, max_features=max_feat, random_state=42, n_jobs=-1
                        )
                        model.fit(X_normal)
...
🚫 NO MORE MOCK MODELS!""",
    ...
    async def train_models(self, only_model: str = None, data_path: str = None):
        ...
        if only_model:
            ...
            if not tasks:
                raise ValueError(
                    f"Model '{only_model}' not found. Available: catboost, xgboost, random_forest, adaptive"
                )
        else:
            tasks = all_tasks
...
async def main():
    ...
    parser.add_argument(
        "--only", choices=["catboost", "xgboost", "random_forest", "adaptive"], help="Train only specified model"
    )
