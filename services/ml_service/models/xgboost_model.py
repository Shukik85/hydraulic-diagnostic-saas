...
class XGBoostModel(BaseMLModel):
    ...
    async def _create_mock_model(self) -> None:
        ...
        try:
            ...
            y_train = np.hstack([np.zeros(int(n_samples * 0.95)), np.ones(int(n_samples * 0.05))])
            ...
            self.model = xgb.XGBClassifier(n_estimators=100, early_stopping_rounds=10, **self.xgb_params)
            ...
            self.model.fit(X_train_scaled, y_train, eval_set=[(X_val, y_val)], verbose=False)
