...
export const useMetadataStore = defineStore('metadata', () => {
    ...
    function addConnection(fromId: string, toId: string, type: 'pressure_line' | 'return_line' | 'pilot_line') {
    ...
    function calculateCompleteness(system: Partial<SystemMetadata>): number {
        ...
        // Level 3: Component details (для каждого компонента)
        const components = system.components || [];
        components.forEach(comp => {
            totalFields += 5; // type, max_pressure, normal_ranges, etc.
