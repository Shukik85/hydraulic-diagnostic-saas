  ...
  <UDialog :model-value="modelValue" @update:model-value="$emit('update:modelValue', $event)">
    <UDialogContent class="max-w-lg">
      ...
      <form @submit.prevent="handleSubmit" class="space-y-6">
        <!-- System Name -->
        <UFormGroup :label="t('systems.create.name')" helper="Используйте понятное имя для идентификации системы"
          :error="errors.name" required>
          <UInput v-model="form.name" :placeholder="t('systems.create.namePlaceholder')" :disabled="loading"
            maxlength="200" />
        </UFormGroup>

        <!-- System Type -->
        <UFormGroup :label="t('systems.create.type')" helper="Тип системы определяет набор доступных параметров">
        ...
        <!-- Initial Status -->
        <UFormGroup :label="t('systems.create.initialStatus')" helper="Статус можно изменить позже в настройках">
          <USelect v-model="form.status" :disabled="loading">
        ...
        <!-- Description -->
        <UFormGroup :label="t('ui.description')" helper="Опишите назначение и особенности системы (опционально)">
          <UTextarea v-model="form.description" :placeholder="t('systems.create.descriptionPlaceholder')"
            :disabled="loading" rows="3" maxlength="500" />
        </UFormGroup>
