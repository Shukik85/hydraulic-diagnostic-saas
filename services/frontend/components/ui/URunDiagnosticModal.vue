  ...
  <UModal :model-value="modelValue" @update:model-value="$emit('update:modelValue', $event)" :title="t('diagnostics.runModal.title')" :description="t('diagnostics.runModal.description')" size="md" :close-on-backdrop="!loading">
    <div class="space-y-5">
      ...
      <div class="relative">
        ...
        <select id="equipment" v-model="form.equipment" class="u-input metallic-select" :disabled="loading">
      ...
      <div class="relative">
        ...
        <select id="diagnostic-type" v-model="form.type" class="u-input metallic-select" :disabled="loading">
      ...
      <div class="space-y-3">
        <div class="flex items-center gap-2">
          <input id="email-notification" v-model="form.emailNotification" type="checkbox" class="w-4 h-4 text-primary-500 bg-steel-dark border-steel-medium rounded focus:ring-primary-500 focus:ring-2" />
          <label for="email-notification" class="text-sm text-text-primary">{{ t('diagnostics.runModal.emailNotification') }}</label>
        </div>
        <div class="flex items-center gap-2">
          <input id="priority-analysis" v-model="form.priorityAnalysis" type="checkbox" class="w-4 h-4 text-primary-500 bg-steel-dark border-steel-medium rounded focus:ring-primary-500 focus:ring-2" />
          <label for="priority-analysis" class="text-sm text-text-primary">{{ t('diagnostics.runModal.priorityAnalysis') }}</label>
        </div>
      </div>
      <!-- Estimated Duration -->
      <div class="rounded-lg bg-success-500/5 border border-success-500/30 p-4">
        <div class="flex items-start gap-3">
          ...
          <div>
            ...
            <p class="text-sm text-success-700 mt-1">{{ getEstimatedDuration() }} - {{ t('diagnostics.runModal.realTimeResults') }}</p>
          </div>
        </div>
      </div>
    </div>
    <template #footer>
      ...
      <button class="u-btn u-btn-primary min-w-[140px]" @click="handleSubmit" :disabled="!isValid || loading" type="button">
