<template>
  <div class="p-8 space-y-10">
    ...
    <section class="bg-white rounded-xl shadow p-6">
      ...
      <div v-else-if="systemStatus.error" class="text-red-600">{{ systemStatus.error.error.message }}</div>
      <div v-else-if="systemStatus.data">
        ...
        <div v-for="comp in systemStatus.data.component_statuses" :key="comp.component_id" class="mt-2">
          <span class="font-medium">Component:</span> {{ comp.type }}
    ...
    <section class="bg-white rounded-xl shadow p-6">
      ...
      <div v-else-if="anomaliesState.error" class="text-red-600">{{ anomaliesState.error.error.message }}</div>
      <div v-else-if="anomaliesState.data">
        ...
        <div class="mt-2">
          <ul>
            <li v-for="anomaly in anomaliesState.data.items" :key="anomaly.prediction_id" :class="getSeverityColor(anomaly.severity)">
              <span class="font-bold">Score:</span> {{ formatAnomalyScore(anomaly.anomaly_score) }}
        ...
        <div class="mt-4">
          ...
          <button @click="nextPage" :disabled="anomaliesPage >= anomaliesState.data.pagination.pages" class="bg-gray-200 py-1 px-3 rounded">Next</button>
          <span class="ml-4">Page {{ anomaliesPage }} of {{ anomaliesState.data.pagination.pages }}</span>
        </div>
      </div>
    </section>
  </div>
...
import { formatAnomalyScore, getSeverityColor, getComponentStatusColor } from '../types/api'
...
watch(severityFilter, (v) => { severity.value = v as any; load() })
