...
class EnhancedTemporalGAT(nn.Module):
    ...
    Key improvements:
    - 3 GAT layers (deeper = better feature learning)
    - LayerNorm (more stable than BatchNorm)
    - Attention Pooling (preserves important information)
    - Residual blocks in classifier
    - Dropout schedule (higher in early layers)
    ...
    def forward(
        ...
        for i, (gat_layer, ln, dropout_p) in enumerate(
            zip(self.gat_layers, self.layer_norms, self.dropout_schedule, strict=False)
        ):
...
class EnhancedHydraulicGNN(nn.Module):
    """Wrapper for enhanced model."""

    def __init__(self):
        super().__init__()
        self.gnn = EnhancedTemporalGAT(
            num_node_features=model_config.num_node_features,
            hidden_dim=model_config.hidden_dim,
            num_classes=model_config.num_classes,
            num_gat_layers=3,  # ✅ 3 layers
...
def create_enhanced_model(device: str = "cuda") -> EnhancedHydraulicGNN:
    """Create and initialize enhanced model."""
    model = EnhancedHydraulicGNN().to(device)
