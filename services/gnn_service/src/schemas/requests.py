...
class TimeWindow(BaseModel):
    ...
    start_time: datetime = Field(..., description="Начало временного окна (ISO 8601)")

    end_time: datetime = Field(..., description="Конец временного окна (ISO 8601)")

    timezone: str = Field(default="UTC", description="Временная зона (IANA timezone name)")
    ...
    def validate_end_after_start(cls, v: datetime, info) -> datetime:
        ...
        if "start_time" in info.data:
            ...
            if delta.days > 30:
                msg = "Time window cannot exceed 30 days"
                raise ValueError(msg)
...
class ComponentSensorReading(BaseModel):
    ...
    pressure_bar: float = Field(..., ge=0, le=1000, description="Pressure in bar")

    temperature_c: float = Field(..., ge=-20, le=150, description="Temperature in °C")

    vibration_g: float | None = Field(
        default=None, ge=0, le=50, description="Vibration level in g (optional)"
    )
    ...
    rpm: float | None = Field(
        default=None, ge=0, le=10000, description="RPM for pumps/motors (optional)"
    )
...
class EdgeOverride(BaseModel):
    ...
    model_config = ConfigDict(json_schema_extra={"example": {"flow_rate_lpm": 118.2}})

    flow_rate_lpm: float | None = Field(
        default=None, ge=0, description="Override flow rate with measured value"
    )

    pressure_drop_bar: float | None = Field(default=None, description="Override pressure drop")

    temperature_delta_c: float | None = Field(
        default=None, description="Override temperature delta"
    )

    vibration_level_g: float | None = Field(
        default=None, ge=0, description="Override vibration level"
    )
...
class MinimalInferenceRequest(BaseModel):
        ...
        json_schema_extra={
            "example": {
                ...
                "sensor_readings": {
                    ...
                    "valve_1": {"pressure_bar": 148.1, "temperature_c": 64.8},
                    "filter_1": {"pressure_bar": 145.0, "temperature_c": 66.0},
                },
                "topology_id": "standard_pump_system",
            }
        }
    ...
    equipment_id: str = Field(
        ..., min_length=1, max_length=100, description="Unique equipment identifier"
    )

    timestamp: datetime = Field(..., description="Timestamp of sensor readings (ISO 8601)")

    sensor_readings: dict[str, ComponentSensorReading] = Field(
        ..., min_length=1, description="Sensor readings per component {component_id: reading}"
    )

    topology_id: str = Field(default="default", description="Pre-configured topology identifier")
    ...
    model_config = ConfigDict(
        json_schema_extra={
            "example": {
                "equipment_id": "pump_system_01",
                "timestamp": "2025-12-03T00:00:00Z",
                "sensor_readings": {"pump_1": {"pressure_bar": 150.2, "temperature_c": 65.3}},
                "topology_id": "standard_pump_system",
                "edge_overrides": {"pump_1->valve_1": {"flow_rate_lpm": 118.2}},
            }
        }
    )
...
class PredictionRequest(BaseModel):
    ...
    equipment_id: str = Field(..., min_length=1, description="Equipment ID")
...
class InferenceRequest(BaseModel):
    ...
    include_recommendations: bool = Field(
        default=True, description="Вернуть рекомендации по обслуживанию"
    )

    confidence_threshold: Annotated[float, Field(ge=0.0, le=1.0)] = Field(
        default=0.7, description="Минимальная confidence для включения результатов"
    )

    custom_parameters: dict[str, str | int | float | bool] = Field(
        default_factory=dict, description="Дополнительные параметры для специфичных use cases"
    )
...
class BatchInferenceRequest(BaseModel):
    ...
    requests: list[InferenceRequest] = Field(
        ..., min_length=1, max_length=100, description="Список inference запросов"
    )

    priority: Literal["low", "normal", "high", "critical"] = Field(
        default="normal", description="Приоритет обработки batch"
    )

    max_parallel: Annotated[int, Field(ge=1, le=10)] = Field(
        default=4, description="Максимальное количество параллельных inference"
    )
    ...
    def validate_unique_equipment_ids(cls, v: list[InferenceRequest]) -> list[InferenceRequest]:
        ...
        if len(equipment_ids) != len(set(equipment_ids)):
            msg = "equipment_id must be unique within batch"
            raise ValueError(msg)
        return v
...
class TrainingRequest(BaseModel):
    ...
    dataset_path: str = Field(..., min_length=1, description="Путь к preprocessed dataset")
    ...
    config_override: dict[str, int | float | bool | str] = Field(
        default_factory=dict, description="Override параметров из SystemConfig"
    )

    use_pretrained: bool = Field(default=False, description="Начать с pretrained weights")

    pretrained_model_path: str | None = Field(
        default=None, description="Путь к pretrained модели (если use_pretrained=True)"
    )
