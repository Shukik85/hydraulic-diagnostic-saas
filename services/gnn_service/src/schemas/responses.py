...
class ComponentPrediction(BaseModel):
    ...
    health: float = Field(
        ..., ge=0.0, le=1.0, description="Component health score (1.0=perfect, 0.0=critical)"
    )

    anomalies: dict[str, float] = Field(
        default_factory=dict, description="Anomaly probabilities per type"
    )
...
class GraphPrediction(BaseModel):
    ...
    health: float = Field(
        ..., ge=0.0, le=1.0, description="Overall system health score (1.0=perfect, 0.0=critical)"
    )
    ...
    rul_hours: float = Field(
        ..., ge=0.0, description="Remaining useful life in hours until predicted failure"
    )

    anomalies: dict[str, float] = Field(
        default_factory=dict, description="System-level anomaly probabilities per type"
    )
...
class PredictionResponse(BaseModel):
    ...
    warnings: list[str] = Field(
        default_factory=list, description="Any warnings or notes about the prediction"
    )
...
class ModelInfo(BaseModel):
    ...
    version: str = Field(
        ..., description="Model version (semantic versioning)", examples=["v2.0.0"]
    )

    architecture: str = Field(
        ..., description="Model architecture name", examples=["UniversalTemporalGNN"]
    )

    num_parameters: int = Field(..., ge=0, description="Total trainable parameters")

    input_features: int = Field(..., ge=1, description="Expected input node feature dimension")

    output_structure: str = Field(
        ..., description="Output structure description", examples=["multi-level: component + graph"]
    )

    training_date: datetime | None = Field(
        default=None, description="Model training completion date"
    )
