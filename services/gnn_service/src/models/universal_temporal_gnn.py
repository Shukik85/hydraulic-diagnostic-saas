...
class ModelConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if self.lstm_num_layers < 1:
            raise ValueError(
                f"lstm_num_layers must be >= 1, got {self.lstm_num_layers}"
            )
...
class UniversalTemporalGNNv2(nn.Module):
    ...
    def __init__(self, config: ModelConfig | None = None) -> None:
        ...
        self.node_encoder = nn.Linear(
            self.config.node_features, self.config.gat_hidden_dim
        )

        # Edge feature encoder
        self.edge_encoder = nn.Linear(
            self.config.edge_features, self.config.gat_hidden_dim
        )
        ...
        for _ in range(self.config.gat_num_layers):
        ...
        if self.config.use_attention_pooling:
            self.pooling = AttentionPooling(
                hidden_dim=self.pooling_input_dim, dropout=self.config.gat_dropout
            )
        else:
            self.pooling = None
        ...
        self.single_graph_projection = nn.Linear(
            self.pooling_input_dim, lstm_output_dim
        )
        ...
        self.component_health_head = nn.Sequential(
            ...
            nn.Linear(
                self.config.head_hidden_dim, self.config.component_health_num_classes
            ),
        )
        ...
        self.anomaly_type_head = nn.Sequential(
            ...
            nn.Dropout(self.config.head_dropout),
            nn.Linear(
                self.config.head_hidden_dim, self.config.anomaly_type_num_classes
            ),
        )

        logger.debug("UniversalTemporalGNNv2 (v%s) initialized", self.config.version)
        logger.debug(
            "GATv2 output dim: %d, LSTM output dim: %d",
            self.gat_output_dim,
            lstm_output_dim,
        )
    ...
    def _encode_nodes(
        self, data, return_attention: bool = False
    ...
    def _forward_single(
        ...
        outputs = {"node_logits": node_logits, "graph_logits": graph_logits}
    ...
    def _forward_temporal(
        ...
        outputs = {"node_logits": last_node_logits, "graph_logits": graph_logits}
