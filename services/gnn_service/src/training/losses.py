...
class WingLoss(nn.Module):
    ...
    def forward(self, pred: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
        ...
        # Wing loss formula
        loss = torch.where(
            delta < self.omega, self.omega * torch.log(1 + delta / self.epsilon), delta - self.C
        )
...
class MultiTaskLoss(nn.Module):
    ...
    def __init__(
        ...
        if weighting == "fixed":
            self.loss_weights = loss_weights or {"health": 1.0, "degradation": 1.0, "anomaly": 1.0}
    ...
    def forward(
        ...
        degradation_loss = self.degradation_loss(degradation_pred, degradation_true)
