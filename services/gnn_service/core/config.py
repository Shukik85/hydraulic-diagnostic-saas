...
class ModelConfig(BaseSettings):
    ...
    # Architecture params
    hidden_dim: int = Field(default=128, ge=32, le=512, description="Hidden dimension size")
    num_heads: int = Field(default=8, ge=1, le=16, description="Number of attention heads")
    num_gat_layers: int = Field(default=3, ge=1, le=6, description="Number of GAT layers")
    lstm_hidden_dim: int = Field(default=256, ge=64, le=1024, description="LSTM hidden size")
    ...
    compile_model: bool = Field(default=True, description="Use torch.compile for optimization")
    compile_mode: Literal["default", "reduce-overhead", "max-autotune"] = Field(default="max-autotune")
    
    # Model paths
    checkpoint_dir: Path = Field(default=Path("checkpoints"))
    model_path: Path = Field(default=Path("models/best_model.tar"))
    onnx_path: Path | None = Field(default=None, description="ONNX export path")
