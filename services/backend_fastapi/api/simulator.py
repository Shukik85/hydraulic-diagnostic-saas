...
async def start_simulator(
    ...
    background_tasks.add_task(
        run_simulation, simulation_id, config, sensor_mappings, equipment.system_id, db
    )
    ...
    return {
        ...
        "data_source_id": data_source.id,
        "status": "started",
        "estimated_readings": len(sensor_mappings)
        * config.duration
...
async def stop_simulator(simulation_id: uuid.UUID):
...
async def run_simulation(
    ...
    try:
        ...
        while (
            ...
            and active_simulations[str(simulation_id)]["status"] == "running"
        ):
...
def generate_sensor_value(
    mapping: SensorMapping, scenario: str, time_elapsed: float, noise_level: float
) -> float:
    """
    ...
    return np.clip(
        value, mapping.expected_range_min * 0.5, mapping.expected_range_max * 1.2
    )
