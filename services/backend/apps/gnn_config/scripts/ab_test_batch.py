...
for i, record in enumerate(dataset[:BATCH_LIMIT]):
    ...
    rows.append({
        ...
        "prod_anomaly_cnt": len(prod_inf.get("anomalies", [])) if isinstance(prod_inf.get("anomalies", []), list) else None,
        "test_anomaly_cnt": len(test_inf.get("anomalies", [])) if isinstance(test_inf.get("anomalies", []), list) else None,
        "prod_summary": prod_diag.get("summary") if isinstance(prod_diag, dict) else str(prod_diag),
        "test_summary": test_diag.get("summary") if isinstance(test_diag, dict) else str(test_diag),
        "prod_reasoning": prod_diag.get("reasoning") if isinstance(prod_diag, dict) else str(prod_diag),
        "test_reasoning": test_diag.get("reasoning") if isinstance(test_diag, dict) else str(test_diag),
    })
...
with open(REPORT_MD, "w", encoding='utf-8') as f:
    f.write(f"# Batch AB-тест моделей GNN + RAG\n")
    ...
    f.write(f"|-----------|-----------|-----------|----------|----------|-------------|-------------|\n")
    for row in rows:
        f.write(f"| {row['equipment_id']} | {row['prod_score']} | {row['test_score']} | {row['prod_anomaly_cnt']} | {row['test_anomaly_cnt']} | {row['prod_summary'][:60]} | {row['test_summary'][:60]} |\n")
