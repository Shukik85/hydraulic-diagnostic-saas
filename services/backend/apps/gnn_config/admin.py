...
class GNNModelConfigAdmin(admin.ModelAdmin):
    list_display = (
        "model_version", "is_active", "deployed_at", "framework", "size_mb", "input_shape", "output_classes", "deployed_by", "prod_status"
    )
    ...
    fieldsets = (
        ("Model Info", {
            "fields": ("model_version", "is_active", "framework", "path", "input_shape", "output_classes", "size_mb", "deployed_at", "deployed_by", "prod_sync_status")
        }),
    ...
    def prod_sync_status(self, obj: GNNModelConfig):
        try:
            ...
            return format_html('<span style="color: #eab308;">Out of sync (v{})</span>', info["model_version"])
    ...
    def deploy_to_production(self, request, queryset):
        for obj in queryset:
            try:
                ...
                obj.is_active = True
                obj.save()
                self.message_user(request, f"Model {obj.model_version} deployed: {res.get('message')}", messages.SUCCESS)
            except Exception as e:
    ...
    def sync_from_service(self, request, queryset):
        ...
        try:
            info = self.client.get_model_info()
            fields = [
                "model_version", "framework", "input_shape", "output_classes", "deployed_at", "size_mb", "model_path"
            ]
            ...
            for obj in queryset:
                ...
                obj.save()
                self.message_user(request, f"Updated {obj.model_version} from prod", messages.SUCCESS)
        except Exception as e:
    ...
    def refresh_status(self, request, queryset):
        for obj in queryset:
            try:
                ...
                obj.save()
                self.message_user(request, f"Status updated: {obj.job_id}={obj.status}", messages.SUCCESS)
            except Exception as e:
                self.message_user(request, f"Status update failed: {str(e)}", messages.ERROR)
    def has_delete_permission(self, request, obj=None):
        return False
