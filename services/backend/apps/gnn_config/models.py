...
class GNNModelConfig(models.Model):
    ...
    path: str = models.CharField(max_length=255, help_text="Filesystem or artifact path to model file")
    ...
    input_shape: str = models.CharField(max_length=64, help_text="Comma-separated shape (e.g., 1,10,32)")
    output_classes: int = models.PositiveSmallIntegerField(default=3)
    deployed_by: User | None = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.SET_NULL, null=True, blank=True)
    description: str = models.TextField(blank=True, default="")
    is_active: bool = models.BooleanField(default=False, help_text="Is this model currently in production?")
    class Meta:
...
class GNNTrainingJob(models.Model):
    ...
    started_by: User | None = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.SET_NULL, null=True, blank=True)
    status: str = models.CharField(max_length=16, choices=[('queued', 'Queued'), ('started', 'Started'), ('running', 'Running'), ('done', 'Done'), ('failed', 'Failed'), ('stopped', 'Stopped')], default='queued', db_index=True)
