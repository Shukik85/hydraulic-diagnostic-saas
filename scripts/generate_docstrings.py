...
class DjangoAwareDocstringGenerator:
    ...
    def analyze_function(self, func_def: ast.FunctionDef, file_path: str) -> Dict:
        ...
        try:
            ...
            if "model" in file_path or "form" in file_path:
                info["returns"] = {"type": "None", "description": "None"}
            elif "view" in file_path and func_def.name == "get":
                info["returns"] = {"type": "HttpResponse", "description": "HTTP ответ"}

            return info
        except Exception as e:
            print(
                f"⚠️  Ошибка анализа функции {getattr(func_def, 'name', 'unknown')}: {e}"
            )
            return {
    ...
    def _infer_django_description(
        self, func_def: ast.FunctionDef, file_path: str
    ) -> str:  # noqa C901
    ...
    def _analyze_django_argument(self, arg: ast.arg, file_path: str) -> Dict:
        ...
        if "view" in file_path.lower():
            if arg_name == "request":
                ...
                return {
                    "name": arg_name,
                    "type": "list",
                    "description": "Позиционные аргументы",
                }
        ...
        if arg_name.endswith("_id"):
            return {
                "name": arg_name,
                "type": "int",
                "description": f"Идентификатор {arg_name[:-3]}",
            }
        elif arg_name.endswith("_list"):
            return {
                "name": arg_name,
                "type": "list",
                "description": f"Список {arg_name[:-5]}",
            }
    ...
    def generate_google_docstring(self, func_info: Dict) -> str:
        ...
        try:
            ...
            if func_info["returns"]:
                ...
                lines.append(
                    f"    {func_info['returns']['type']}: {func_info['returns']['description']}"
                )
                lines.append("")
...
def find_django_py_files(directory: str) -> List[Path]:
    ...
    print(
        f"📁 Найдено {len(py_files)} .py файлов в {directory} (исключены миграции и системные файлы)"
    )
    return py_files
...
def ensure_function_has_body(
    lines: List[str], func_node: ast.FunctionDef, indent: int
) -> List[str]:
...
def process_django_file(
    file_path: Path, generator: DjangoAwareDocstringGenerator
) -> bool:  # noqa C901
            ...
            try:
                ...
                if existing_docstring is None:
                    ...
                    if def_line_idx < len(lines):
                        indent = len(lines[def_line_idx]) - len(
                            lines[def_line_idx].lstrip()
                        )
